...
Suppose on your last visit to the doctor’s office you took a 
...
the signal is described by the prior distribution
%
\be
...
\item
show that the sampling distributions of the test statistic are
%
\begin{align}
...
\ee
%
is the non-centrality parameter for a non-central
...
\ee
%
but these values can be changed to consider
stronger (or weaker) injections, etc.

For a realization of simulated data, calculate 
the following quantities, making plots when possible:
%
\ben
\item
...
$\Lambda(d)$ and its corresponding $p$ value.

\item
the frequentist detection probability curve,
...
\item
the Bayes factor ${\cal B}_{10}(d)$, twice the log of the Bayes 
factor $2\ln {\cal B}_{10}(d)$, and
the Laplace approximation of that quantity, see Eq.~\eqref{e:laplace}.
