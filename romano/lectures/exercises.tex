...
Consider a constant amplitude signal $a={\rm const}$ in 
white Gaussian noise with variance 
$\sigma^2$ described by the likelihood functions
%
\be
...
\ee
%
For the Bayesian analyses, use the following prior for $a$:
%
\be
p(a|{\cal M}_1) = \frac{1}{a_{\rm max}}
\ee

Perform both frequentist and Bayesian analyses for the
above signal and noise models, doing both analytic and
numerical calculations of relevant quantities.

\be
N=100\,,\quad \sigma=1\,,\quad  
0\le a \le a_{\rm max}\,, \quad a_0 =\text{true value}
\ee
